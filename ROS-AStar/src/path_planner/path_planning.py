...
class PathPlanner:
    def __init__(self):
        ...
        # self.mover = path_planner.GotoMover(self)                                  -> change to PPC
        ...
        self.path_publisher = rospy.Publisher("/target_path", Path, queue_size=10)
        # self.mover_publisher = rospy.Publisher("/move", Path, queue_size=1)                           # this is the cause for the movement
    ...
    def wait_for_result(self, duration: float) -> bool:
        ...
        while not self.is_goal_cancelled and not rospy.is_shutdown():              
